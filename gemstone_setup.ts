...
async function setupGemstoneMarketplace() {
  try {
    console.log('Setting up gemstone marketplace...');
    ...
    console.log('Creating gemstone_categories table...');
    await db.execute(sql`
    ...
    console.log('Creating gemstones table...');
    await db.execute(sql`
    ...
    console.log('Checking for existing GemsBiz supplier...');
    const existingSuppliers = await db.execute(sql`
      SELECT * FROM suppliers WHERE name = 'GemsBiz';
    `);
    ...
    if (existingSuppliers.rows.length === 0) {
      ...
      supplierId = existingSuppliers.rows[0].id;
    }
    console.log(`Using supplier ID: ${supplierId}`);

    // Add gemstone categories if they don't exist
    ...
    for (const category of categories) {
      ...
      const existingCategory = await db.execute(sql`
        SELECT * FROM gemstone_categories WHERE name = ${category.name};
      `);
    ...
    console.log('Gemstone categories created');

    // Add sample gemstones
    console.log('Adding sample gemstones...');
    const gemstones = [
      {
    ...
    for (const gemstone of gemstones) {
      ...
      const existingGemstone = await db.execute(sql`
        SELECT * FROM gemstones WHERE name = ${gemstone.name};
      `);

      if (existingGemstone.rows.length === 0) {
        console.log(`Adding gemstone: ${gemstone.name}`);
        await db.execute(sql`
          INSERT INTO gemstones (
            ...
            ${supplierId}, 
            ${categoryIds[gemstone.categoryName]}, 
            ${gemstone.price}, 
        ...
        console.log(`Gemstone already exists: ${gemstone.name}`);
      }
    }

    console.log('Gemstone marketplace setup completed successfully!');
  } catch (error) {
    console.error('Error setting up gemstone marketplace:', error);
  }
}

setupGemstoneMarketplace();